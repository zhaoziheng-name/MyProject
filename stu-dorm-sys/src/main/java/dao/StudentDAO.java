...
public class StudentDAO {
    ...
    public static List<Student> query(Page p) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                int count = rs.getInt("count"); // 设置到返回数据的 total 字段,当前方法是无法通过返回对象设置
                // 使用 ThreadLocal: 变量绑定到线程 Thread 类的 ThreadLocalMap 类型的属性中
            ...
            // 页码转换为索引: 上一页的页码 * 每页的数量, 注意索引是从 0 开始
    ...
    public static int apply(Student s) {
        ...
        try {
            ...
            throw new RuntimeException("分配学生出错", e);
        } finally {
            DBUtil.close(c, ps);
        }
    }
